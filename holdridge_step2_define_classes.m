...
fileName_tabulated_present = sprintf('holdridge_data/tabulated_holdridge_results_present_%s.mat',date );
csvwrite(fileName_tabulated_present,hold_classes);
...
fileName_tabulated_7_present = sprintf('holdridge_data/tabulated_holdridge_results_7_present_%s.mat',date );
csvwrite(fileName_tabulated_7_present,hold_classes7);
...
v_data_hold_future = zeros(size(hIndex,1),size(data_holdridge_future,3),size(data_holdridge_future,4)...
...
hold_classes_fut = zeros(38,3,size(data_holdridge_future,4)...
    ,size(data_holdridge_future,5),'single');
...
%% save
save(file_resultsFuture, 'v_*_fut*', 'hLand', 'hIndex', 'hold_class*_fut*')
