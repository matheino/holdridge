...
% download data and calculate parameters (1) 
...
[num,urlList,raw] = xlsread(...
    'input/worldclim_data_download.xlsx','current','D18:D21');
clearvars num raw
...
for i = 1:size(urlList,1)
    ...
    fileUrl = urlList{i,1};
    tempUrl = websave(fileNameZip,fileUrl);
...
[temp_data_out] = f_holdridge_parameters_no_Tmax_cap(temp_data,bias_correction_tavg);
...
[num,urlList,raw] = xlsread(...
    'input/worldclim_data_download.xlsx',future_sheet,'I7:H77');
clearvars num raw
...
data_holdridge_future = zeros(2160,4320,4,2,8,'single'); % lat, lon, holdridge parameter, RCP, GCM
...
for gcm = 1:8
    ...
    for rcp = 1:2
        ...
        for var = 1:3 % we do not need the 4th dimension, i.e. bio variables
            ...
            fileNameZip = 'temp.zip';
            fileUrl = urlList{(gcm-1)*8 + (rcp-1)*4 + var,1};
...
for gcm = 1:8
    ...
    for rcp = 1:2
        ...
        for var = 1:3
            ...
            fileName = getfield(listing,{(gcm-1)*6 + (rcp-1)*3 + (4-var),1}, 'name');
            
            folder = getfield(listing,{(gcm-1)*6 + (rcp-1)*3 + (4-var),1}, 'folder');
...
% figure; imagesc(data_holdridge_present(:,:,4))
% 
% figure; imagesc(nanmean(data_holdridge_future(:,:,1,1,:),5))
% figure; imagesc(nanmean(data_holdridge_future(:,:,2,1,:),5))
% figure; imagesc(nanmean(data_holdridge_future(:,:,3,1,:),5))
% figure; imagesc(nanmean(data_holdridge_future(:,:,4,1,:),5))
% 
% figure; imagesc(nanmean(data_holdridge_future(:,:,1,2,:),5))
% figure; imagesc(nanmean(data_holdridge_future(:,:,2,2,:),5))
% figure; imagesc(nanmean(data_holdridge_future(:,:,3,2,:),5))
